...
async def send_info(message: types.Message):
    await message.answer(
        """
🔹 Хто ми?  
Допомагаємо заробити на верифікаціях (KYC).

🔹 Як це працює?  
1. Заходиш в акаунт за інструкцією
2. Проходиш KYC
3. Отримуєш оплату сразу

🔹 Законно?  
Так. Це не шахрайство.

🔹 Скільки платимо?  
100–400 грн за заявку
""",
        parse_mode="Markdown")
...
2. *Як оплата?*
   - На карту/крипту після підтвердження

3. *Скільки часу?*
   - 10–20 хвилин

4. *Чи безпечно?*
   - Так, ми тільки з офіційними біржами
""", parse_mode="Markdown")

@dp.message_handler(text='📝 Заповнити заявку')
async def start_application(message: types.Message):
    await message.answer("1. Ім’я та нік в Telegram:", reply_markup=ReplyKeyboardRemove())
...
async def process_city(message: types.Message, state: FSMContext):
    ...
    await message.answer("4. Які документи маєш?")
    await ApplicationStates.next()
...
async def process_exp(message: types.Message, state: FSMContext):
    ...
    await message.answer("5. Чи вже проходив верифікації? ")
    await ApplicationStates.next()
...
async def process_phone(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        try:
            ...
            await message.answer(
                "✅ Дякуємо! Долучайтесь до групи з завданнями:\n👉 https://t.me/+06666cc2_TwwMDZi",
